...
| # | Item | Notes |
| --- | --- | --- |
| 1 | Residual sanitizer garbage | **Promoted from cosmetic: it now lands inside stored signatures.** Confirmed live on 2026-08-11: `Authorization token is requiredb`, `Unsupported protocol hhttps:`. **Working hypothesis:** Docker frames each log line with an 8-byte header — one stream byte, three zeros, four big-endian length bytes — and `PRINTABLE_ASCII_PATTERN` (`src/utils/sanitize-log-line.ts:4`) drops only the non-printable ones. The low length byte survives when it happens to be printable: `b`=0x62=98, `h`=0x68=104, `r`=0x72=114, all plausible line lengths. If that holds, the fix strips the header by its structure *before* control characters are removed, rather than filtering stray characters — which would corrupt legitimate text. **Blocked on evidence:** run `{"diagnostic":"loki","job":"docker_prms_test"}`, which returns unsanitized lines, and confirm the byte layout before writing the pattern. |
...
| 6 | Bedrock response parsing fails on roughly one alert in six | Observed 2026-08-11: 1 of 6 stored alerts carried `bedrockFallbackReason: "Unexpected non-whitespace character after JSON at position 466"`. The alert still reached Slack, as the invariant requires, but unenriched. `extractJsonObject` (`src/bedrock/structured-output-validator.ts:39`) is meant to tolerate prose around the JSON object and evidently does not handle this shape — likely trailing content after a complete object. This no longer corrupts signatures (fixed in `4ba6e83`), so it now costs enrichment only. Worth reproducing with a real captured response before changing the parser. |
